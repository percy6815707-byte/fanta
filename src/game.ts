...
(() => {
  ...
  const state = {
    ...
    pendingResonancePlacement: null,
  ...
  function clearPendingResonancePlacement() {
    state.pendingResonancePlacement = null;
  }

  function resonanceIndexToCoord(index) {
    if (!Number.isInteger(index) || index < 0 || index >= 9) return null;
    return { x: index % 3, y: Math.floor(index / 3) };
  }

  function resonanceCoordToIndex(x, y) {
  ...
  function renderResonanceBoard() {
    ...
    const pending = state.pendingResonancePlacement;
    const pendingSpell = pending ? spellLibrary[pending.spellId] : null;
    if (pending && !pendingSpell) {
      clearPendingResonancePlacement();
    }
    ...
    for (let i = 0; i < 9; i += 1) {
      ...
      if (pendingSpell) {
        const pattern = resonancePatternForSpellAtCenter(pendingSpell, i);
        const triggerCount = pattern.reduce((count, idx) => {
          const existing = state.resonanceBoard[idx];
          return existing && existing.color ? count + 1 : count;
        }, 0);
        const detail = [
          `공명 위치 선택: ${pendingSpell.name}`,
          `중심 칸: ${i + 1}`,
          `각인 적용 칸: ${pattern.map((idx) => idx + 1).join(", ")}`,
          `즉시 발동 예상: ${triggerCount}칸`
        ].join("\n");
        cell.classList.add("selectable");
        if (i === 4) cell.classList.add("center");
        if (pattern.includes(i)) cell.classList.add("preview");
        cell.tabIndex = 0;
        cell.title = detail.replace(/\n/g, " | ");
        cell.addEventListener("mouseenter", () => setBattleActionDetail(detail));
        cell.addEventListener("focus", () => setBattleActionDetail(detail));
        cell.addEventListener("click", () => resolvePendingResonancePlacement(i));
        cell.addEventListener("touchstart", () => setBattleActionDetail(detail), { passive: true });
        cell.addEventListener("keydown", (event) => {
          if (event.key !== "Enter" && event.key !== " ") return;
          event.preventDefault();
          resolvePendingResonancePlacement(i);
        });
      }
    ...
    if (dom.resonanceHint) {
      if (pendingSpell) {
        const bestHits = estimateBestResonanceTriggerCount(pendingSpell);
        dom.resonanceHint.textContent = `공명 위치 선택 중: ${pendingSpell.name} · 중심 칸을 선택하세요 (최대 발동 ${bestHits}칸)`;
      } else if (activeMarks <= 0) {
  ...
  function resonancePatternForSpellAtCenter(spell, centerIndex = 4) {
    const base = resonancePatternForSpell(spell);
    const sourceCenter = resonanceIndexToCoord(4);
    const targetCenter = resonanceIndexToCoord(centerIndex);
    if (!sourceCenter || !targetCenter) return [];
    const applied = [];
    base.forEach((idx) => {
      const point = resonanceIndexToCoord(idx);
      if (!point) return;
      const tx = targetCenter.x + (point.x - sourceCenter.x);
      const ty = targetCenter.y + (point.y - sourceCenter.y);
      const targetIndex = resonanceCoordToIndex(tx, ty);
      if (targetIndex < 0) return;
      if (!applied.includes(targetIndex)) applied.push(targetIndex);
    });
    return applied;
  }

  function applyPlayerSpellResonance(spell, centerIndex = 4) {
    const pattern = resonancePatternForSpellAtCenter(spell, centerIndex);
  ...
  function pendingResonanceTriggerCount(spell, centerIndex = 4) {
    const pattern = resonancePatternForSpellAtCenter(spell, centerIndex);
  ...
  function estimateBestResonanceTriggerCount(spell) {
    let best = 0;
    for (let idx = 0; idx < 9; idx += 1) {
      best = Math.max(best, pendingResonanceTriggerCount(spell, idx));
    }
    return best;
  }
  ...
  function performPlayerSpellTurn(slotIndex, resonanceCenterIndex = 4) {
    ...
    const resonanceSummary = applyPlayerSpellResonance(spell, resonanceCenterIndex);
  ...
  function beginPendingResonancePlacement(slotIndex) {
    if (!canPlayerAct()) return false;
    ...
    const pending = state.pendingResonancePlacement;
    if (pending && pending.slotIndex === slotIndex && pending.spellId === spellId) {
      clearPendingResonancePlacement();
      setBattleActionDetail(recommendedPlayerActionDetail());
      updateUI();
      return true;
    }
    state.pendingResonancePlacement = { slotIndex, spellId };
    setBattleActionDetail([
      `${spell.name} 시전 준비`,
      "3x3 공명 그리드에서 중심 칸을 선택하세요.",
      "같은 슬롯 버튼을 한 번 더 누르면 취소됩니다."
    ].join("\n"));
    updateUI();
    return true;
  }

  function resolvePendingResonancePlacement(centerIndex) {
    const pending = state.pendingResonancePlacement;
    ...
    if (!Number.isInteger(centerIndex) || centerIndex < 0 || centerIndex >= 9) return false;
    const spellId = player.spellSlots[pending.slotIndex];
    if (!spellId || spellId !== pending.spellId) {
      clearPendingResonancePlacement();
      updateUI();
      return false;
    }
    clearPendingResonancePlacement();
    return performPlayerSpellTurn(pending.slotIndex, centerIndex);
  }
  ...
  function performPlayerActionSlot(slotIndex) {
    ...
    return beginPendingResonancePlacement(slotIndex);
  }
    ...
    function bindEditorListeners() {
      ...
      dom.spellSlots.addEventListener("click", (event) => {
        ...
        if (state.mode === "running" && TURN_BASED_COMBAT) {
          ...
          if (!entry) return;
          event.stopPropagation();
          performPlayerActionSlot(entry.slotIndex);
          return;
        }
  ...
  function renderBattleActionBar() {
    ...
    const setupSlotBtn = (button, slotIndex) => {
      ...
      const resonanceHits = estimateBestResonanceTriggerCount(spell);
      ...
      if (subNode) {
        ...
        else if (resonanceHits > 0) subNode.textContent = `공명 최대 ${resonanceHits}칸 발동`;
  ...
  function spellDetailSummary(spell) {
    ...
    const basePattern = resonancePatternForSpell(spell);
    const triggerCount = estimateBestResonanceTriggerCount(spell);
    lines.push(`공명 패턴 칸수: ${basePattern.length} (중심 선택형)`);
    lines.push(`현재 즉시 발동 최대: ${triggerCount}칸`);
  ...
  function recommendedPlayerActionDetail() {
    ...
    const pending = state.pendingResonancePlacement;
    if (pending) {
      const spell = spellLibrary[pending.spellId];
      if (spell) {
        return [
          `${spell.name} 시전 준비`,
          "공명 3x3에서 중심 칸을 선택해 시전하세요.",
          "같은 슬롯 버튼을 다시 누르면 취소됩니다."
        ].join("\n");
      }
    }
    let best = null;
